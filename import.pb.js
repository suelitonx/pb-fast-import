...
onRecordAfterCreateSuccess((e) => {
    ...
    let col = e.record.collection().baseFilesPath();

    let baseFolder = $os.getwd() + '\\pb_data\\storage\\' + col + '\\' + e.record.id;

    let folderFiles = $os.readDir(baseFolder)

    //let colName = e.record.getString("")
    let colName = e.record.getString("collection_name")
    ...
    for (let i = 0; i < folderFiles.length; i++) {
        ...
        if(name.endsWith(".csv") && !(name.includes('.attrs')) )
        {
            let fileName = (baseFolder + '/' + name).replaceAll(/\\/g, '/');
